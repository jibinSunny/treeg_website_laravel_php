...
<div class="row wrapper border-bottom white-bg page-heading">
    ...
    <div class="col-lg-2">
            ...
            <a href="{{ asset('admin/projects/create') }}" class="mt-2 btn btn-outline btn-primary"><i class="fa fa-plus"></i> Add New Project</a>
            @endpermission
    </div>
</div>
        ...
        <div class="ibox float-e-margins">
            ...
            @foreach ($projects as $project)
                <tr class="gradeX">
                    ...
                    <td>{{ $project->title }}</td>
                    <td>{{ ($project->category_id==1) ? 'Residential' : 'Commercial' }}</td>
                    <td>@foreach($types as $typesdf)
                        @if($typesdf->id == $project->type)
                        {{$typesdf->name }}
                        @endif
                        @endforeach
                    </td>
                    ...
                    <td><span class="label label-{{ ($project->active==true) ? 'primary': 'danger' }}">{{ ($project->active==true) ? 'Active': 'Inactive' }}</span></td>
                   
                    <td>
                        @permission('create.projects')
                        <a href="{{ asset('admin/projects/'.$project->id) }}" class="btn table-action-btn btnViewRow" title="View"> <i class="fa fa-eye"></i></a>
                        @endpermission
                        @permission('edit.projects')

                        <a href="{{ asset('admin/projects/'.$project->id.'/edit') }}" class="btn table-action-btn " title="Edit"><i class="fa fa-pencil-square-o"></i> </a> 
                        @endpermission
                        @permission('delete.projects')


                        <a href="" data-url="{{ route('projects.destroy',$project->id) }}" class="btn table-action-btn btnDeleteRow" title="Delete"> <i class="fa fa-times-circle"></i></a> 
