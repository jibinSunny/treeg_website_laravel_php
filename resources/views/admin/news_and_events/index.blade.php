...
<div class="row wrapper border-bottom white-bg page-heading">
    ...
    <div class="col-lg-2">
            ...
            <a href="{{ asset('admin/newsandevents/create') }}" class="btn btn-primary btn-outline">Add New</a>
            @endpermission
    </div>
</div>
...
<div class="wrapper wrapper-content animated fadeInRight">
    <div class="row">
        ...
        <div class="ibox float-e-margins">
            ...
            @foreach ($newsandEvents as $i=> $newsandevent)
                <tr class="gradeX">
                      <td>{{ ($newsandEvents->currentpage()-1) * $newsandEvents->perpage() + $i + 1 }}</td>
                    <td>{{ $newsandevent->title }}</td>
                     <td><img src="{{ asset(Storage::url($newsandevent->image)) }}" width="40" alt=""></td>
                   
                    <td>{{ $newsandevent->date }} </td>
                    <td>{{ str_limit($newsandevent->description,50) }}</td>
                    <td>
                        @permission('view.newsandevents')
                          <a href="{{ asset('admin/newsandevents/'.$newsandevent->id) }}" class="btn table-action-btn btnViewRow" title="View"> <i class="fa fa-eye"></i></a>
                        @endpermission
                        @permission('edit.newsandevents')
                        <a href="{{ asset('admin/newsandevents/'.$newsandevent->id.'/edit') }}" class="btn table-action-btn " title="Edit"><i class="fa fa-pencil-square-o"></i> </a> 
                        @endpermission
                        @permission('delete.newsandevents')

                        <a href="" data-url="{{ route('newsandevents.destroy',$newsandevent->id) }}" class="btn table-action-btn btnDeleteRow" title="Delete"> <i class="fa fa-times-circle"></i></a> 
